...
<AccordionGroup>
  <Accordion title="agent-vault server">
    ...
    **Environment variables:**
    ...
    | `AGENT_VAULT_SKIP_CLI_INSTALL` | Set to `true` to skip the post-registration CLI installation screen and continue directly to the app or login. |
    | `AGENT_VAULT_DEFAULT_SERVICES_JSON` | JSON array of service objects added by name to existing vaults at startup and to newly created built-in vaults. Existing same-name services are preserved; no credentials are created. Invalid JSON or service definitions prevent server startup. |
    | `AGENT_VAULT_OAUTH_GOOGLE_CLIENT_ID` | Client ID for an instance-managed Google OAuth Web application. Set together with `AGENT_VAULT_OAUTH_GOOGLE_CLIENT_SECRET`. |
    | `AGENT_VAULT_OAUTH_GOOGLE_CLIENT_SECRET` | Client secret for an instance-managed Google OAuth Web application. Set together with `AGENT_VAULT_OAUTH_GOOGLE_CLIENT_ID`; the value is unset from the process environment after startup. |
    | `AGENT_VAULT_OAUTH_GITHUB_CLIENT_ID` | Client ID for an instance-managed GitHub App user authorization flow. Set together with `AGENT_VAULT_OAUTH_GITHUB_CLIENT_SECRET`. |
    | `AGENT_VAULT_OAUTH_GITHUB_CLIENT_SECRET` | Client secret for an instance-managed GitHub App. Set together with `AGENT_VAULT_OAUTH_GITHUB_CLIENT_ID`; the value is unset from the process environment after startup. |
    | `AGENT_VAULT_MINDROOM_GITHUB_APP_ID` | Positive decimal ID for the operator-owned MindRoom GitHub App. All seven `AGENT_VAULT_MINDROOM_*` variables are required together. |
    | `AGENT_VAULT_MINDROOM_GITHUB_APP_INSTALLATION_ID` | Positive decimal ID for the App's selected-repository organization installation. |
    | `AGENT_VAULT_MINDROOM_GITHUB_APP_ORGANIZATION_ID` | Positive decimal GitHub organization account ID used to pin the installation identity. |
    | `AGENT_VAULT_MINDROOM_GITHUB_ORGANIZATION` | Exact organization login allowed for MindRoom-owned repositories. |
    | `AGENT_VAULT_MINDROOM_GITHUB_REPOSITORY_PREFIX` | Exact prefix required on complete repository names supplied by MindRoom. MindRoom remains responsible for deriving each name. |
    | `AGENT_VAULT_MINDROOM_GITHUB_APP_PRIVATE_KEY_FILE` | Path to the App's unencrypted RSA private key in PKCS#1 or PKCS#8 PEM format. The key is read from the file and never stored in a logical vault. |
    | `AGENT_VAULT_MINDROOM_REPOSITORY_BROKER_TOKEN_FILE` | Path to the dedicated MindRoom control-plane bearer token file. The token must be at least 32 bytes and is not an ordinary Agent Vault session token. See [MindRoom GitHub repositories](/self-hosting/mindroom-github-repositories). |
...
<AccordionGroup>
  <Accordion title="agent-vault vault credential list">
    ...
    |------|---------|-------------|
    | `--vault` | `default` | Target vault |
    | `--reveal` | `false` | Show decrypted static and dynamic credential values (requires member+ role). OAuth tokens remain broker-only. |
  </Accordion>
  ...
  <Accordion title="agent-vault vault credential get">
    ...
    Print the decrypted value of a single static or dynamic credential to stdout. Pipe-friendly. OAuth credentials are rejected because their tokens remain broker-only. Requires member+ role. Alias: `agent-vault vault creds get`. In agent mode (`AGENT_VAULT_TOKEN` set), `AGENT_VAULT_VAULT` (or `--vault`) is required — there is no project-file or interactive-picker fallback.
