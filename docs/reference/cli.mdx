...
<AccordionGroup>
  <Accordion title="agent-vault server">
    ...
    **Environment variables:**
    ...
    | `AGENT_VAULT_UI_BASE_PATH` | Browser UI mount path (default `/`). For example, `/vault` serves the UI, assets, browser API calls, deep links, invitations, approvals, and OAuth UI redirects below `/vault/`. A leading slash is optional and one trailing slash is removed. Invalid external, encoded, traversing, or ambiguous paths prevent startup. Root control API endpoints and the separate proxy listener remain unchanged. |
