...
<AccordionGroup>
  <Accordion title="agent-vault server">
    ...
    | `--ui-base-path` | `""` (domain root) | Browser UI path prefix, e.g. `/vault`, for a reverse proxy on a shared domain. Browser API calls use the prefix; root `/v1` and `/discover` control APIs stay available. The proxy must pass paths through unmodified. `GET /` redirects to the prefix. Also respects `AGENT_VAULT_UI_BASE_PATH`; the flag takes precedence. See [Serving under a path prefix](/self-hosting/path-prefix). |
    ...
    **Environment variables:**
    ...
    | `AGENT_VAULT_UI_BASE_PATH` | Browser UI path prefix (default empty = domain root). Root control APIs remain available. The `--ui-base-path` flag takes precedence. See [Serving under a path prefix](/self-hosting/path-prefix). |
