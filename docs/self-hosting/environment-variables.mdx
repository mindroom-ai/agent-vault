...
| `AGENT_VAULT_UI_BASE_PATH` | Optional (defaults to empty = domain root) | Browser UI path prefix (e.g., `/vault`) for reverse-proxy deployments on a shared domain. Browser API calls also use the prefix; root `/v1` and `/discover` control APIs stay available. The proxy must pass paths through unmodified. Overridden by `--ui-base-path`. See [Serving under a path prefix](/self-hosting/path-prefix). |
