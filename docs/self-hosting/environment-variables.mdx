...
| `AGENT_VAULT_DEFAULT_SERVICES_JSON` | Optional | JSON array of service objects to add to every vault. Agent Vault validates the array before startup, backfills missing names in existing vaults, and seeds newly created built-in vaults. Existing same-name services are preserved; credentials are never created. |
