...
| `AGENT_VAULT_UI_BASE_PATH` | Optional (defaults to `/`) | Mounts the browser UI and its browser API requests below a local URL path such as `/vault`. A leading slash is optional and one trailing slash is removed, so `vault`, `/vault`, and `/vault/` are equivalent. External URLs, traversal, encoding, queries, fragments, backslashes, repeated separators, and prefixes overlapping `/v1`, `/health`, or `/discover` are rejected at startup. The root control API and separate proxy listener keep their existing addresses. |
...
Register `{AGENT_VAULT_ADDR}{AGENT_VAULT_UI_BASE_PATH}/v1/oauth/callback` as an authorized callback for each configured provider. Omit the middle value when the UI base path is `/`; for `/vault`, the callback ends in `/vault/v1/oauth/callback`. Both variables in a provider pair are required; partial configuration prevents startup.
