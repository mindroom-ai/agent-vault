...
| `AGENT_VAULT_SKIP_CLI_INSTALL` | Optional (defaults to `false`) | Set to `true` when users access only the hosted web UI. Successful registration then continues directly to the app or login instead of showing the CLI installation step. |
| `AGENT_VAULT_DEFAULT_SERVICES_JSON` | Optional | JSON array of service objects. Agent Vault validates the array before startup, backfills missing names in all existing vaults at startup, and seeds newly created built-in vaults. Newly created external-store vaults are not seeded during creation. Existing same-name services are preserved; credentials are never created. |
...
An instance operator can configure shared Google OAuth and GitHub App clients. Vault users then choose a managed provider and authorize their own account without creating or entering OAuth client credentials. Google users select scopes. GitHub App user authorization sends no classic OAuth scopes; effective access is limited by the app installation, the app's configured permissions, and the user's own access. Access and refresh tokens remain encrypted and separate per vault. Shared client secrets are resolved from current server configuration during authorization and refresh.

Register `{AGENT_VAULT_ADDR}/v1/oauth/callback` as an authorized callback for each configured provider. Both variables in a provider pair are required; partial configuration prevents startup.
...
| `AGENT_VAULT_OAUTH_GITHUB_CLIENT_ID` | Conditional | Client ID for the instance-managed GitHub App user authorization flow. |
| `AGENT_VAULT_OAUTH_GITHUB_CLIENT_SECRET` | Conditional | Client secret for the instance-managed GitHub App. Source it from the deployment platform's secret store. |

GitHub API services use the per-user access token as a bearer credential. Existing Git smart-HTTP services can use the same credential as their HTTP password, so choosing the default `GITHUB_TOKEN` key replaces a PAT without adding a second transport. No GitHub App private key or installation-token minting is involved. This server configuration does not modify downstream tool or deployment configuration.

## MindRoom repository machine identity

MindRoom repository brokering uses a separate operator-owned GitHub App machine identity. It does not replace or broaden the requester-scoped managed GitHub OAuth flow above. Set all seven variables to enable it; leaving all seven unset disables it, while partial configuration prevents startup.

| Variable | Required | Description |
|----------|----------|-------------|
| `AGENT_VAULT_MINDROOM_GITHUB_APP_ID` | Conditional | Positive decimal GitHub App ID, sourced from Secret key `app-id`. |
| `AGENT_VAULT_MINDROOM_GITHUB_APP_INSTALLATION_ID` | Conditional | Positive decimal organization installation ID, sourced from Secret key `installation-id`. |
| `AGENT_VAULT_MINDROOM_GITHUB_APP_ORGANIZATION_ID` | Conditional | Positive decimal GitHub organization account ID, sourced from Secret key `organization-id`. |
| `AGENT_VAULT_MINDROOM_GITHUB_ORGANIZATION` | Conditional | Exact allowed organization login, configured as `example-org`. |
| `AGENT_VAULT_MINDROOM_GITHUB_REPOSITORY_PREFIX` | Conditional | Required repository-name prefix, configured as `MindRoom-`. MindRoom remains responsible for deriving the complete name. |
| `AGENT_VAULT_MINDROOM_GITHUB_APP_PRIVATE_KEY_FILE` | Conditional | Path to the App's unencrypted RSA private key in PKCS#1 or PKCS#8 PEM format. |
| `AGENT_VAULT_MINDROOM_REPOSITORY_BROKER_TOKEN_FILE` | Conditional | Path to the separate control-plane bearer token. The trimmed token must be at least 32 bytes. |

See [MindRoom GitHub repositories](/self-hosting/mindroom-github-repositories) for the exact GitHub App, Kubernetes Secret, broker API, proxy, health, and image contracts.
